...
class Game:
    ...
    def ai_move(self):
        if self.turn == BLUE:
            if self.phase == "placing":
                available_nodes = [node for node in NODES.keys() if node not in self.placed_pieces[ORANGE] and node not in self.placed_pieces[BLUE]]
                if available_nodes:
                    chosen_node = random.choice(available_nodes)
                    self.pieces[BLUE][self.pieces[BLUE].index(None)] = chosen_node
                    self.placed_pieces[BLUE].append(chosen_node)
                    self.turn = ORANGE  # Switch turn after AI move
                ...
                piece = random.choice(self.placed_pieces[BLUE])
                possible_moves = [m for m in VALID_MOVES[piece] if m not in self.placed_pieces[ORANGE] and m not in self.placed_pieces[BLUE]]
                if possible_moves:
                    self.moving_piece = piece
                    self.moving_target = random.choice(possible_moves)
                    self.moving_position = NODES[piece]
    ...
    def run(self):
        while self.running:
            ...
            if self.turn == BLUE and not self.moving_piece:
                self.ai_move()  # Now AI places in both placing & moving phases
